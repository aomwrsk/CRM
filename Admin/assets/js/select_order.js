function fetchset() {
  ...
  if (selectedRadio) {
    ...
    const orNo = selectedRadio.getAttribute('data-or-no');
    // Set the value of the input field with the selected OW No.
    document.getElementById('Is_No').value = owNo;
    let url;

    url = `fetch_is.php?issue_no=${owNo}&or_no=${orNo}&set=${set_type}`;
  fetch(url)
  .then(response => {
    if (!response.ok) {
  ...
  .then(data => {
    ...
    console.log('Selected OR No:', orNo);
...
const formattedDate = dateObj.getFullYear() + '-' + 
  ...
  const modalElement = document.getElementById('ExtralargeModal1');
...
function updateData(data) {
  ...
  data.waste.forEach((row, index) => {
    ...
    tr.innerHTML = `
      ...
      <td><input type="checkbox" class="form-check-input"  name="gridRadios" id="gridRadios${index + 1}"style="margin-left: 20%;"></td>
...
document.getElementById('confirmBtn1').addEventListener('click', fetchset);